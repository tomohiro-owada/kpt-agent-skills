...
- **全プロジェクト横断** — 一度インストールすれば、どのプロジェクトで Claude Code を使っても自動的にログが記録されます。振り返り時はプロジェクトごとにグルーピングして表示
...
```
/plugin install tomohiro-owada/kpt-agent-skills
```
...
```bash
git clone https://github.com/tomohiro-owada/kpt-agent-skills.git
cd kpt-hooks
./install.sh
```
